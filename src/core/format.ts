...
export function parseIndexOutput(stdout: string): ZgIndexSummary | undefined {
    const files = stdout.match(
        /^files\t(\d+) scanned, (\d+) added, (\d+) modified, \d+ retried, (\d+) unchanged, (\d+) deleted, \d+ failed$/m,
    );
...
export function parseStatusVerdict(stdout: string): ZgStatusVerdict | undefined {
    ...
    if (/needs an? update/.test(first)) {
        ...
        return {
            kind: "needs-update",
            line: `index stale${bits.length > 0 ? ` · ${bits.join(" ")}` : ""} — run zvec_index`,
        };
    }
