        ...
        (_tui, theme, _kb, done) => {
            ...
            const list = new SettingsList(
                ...
                (id, value) => {
                    ...
                    if (id === "projectScope") {
                        ...
                        try {
                            if (value === "project") {
                                ...
                                if (created)
                                    ctx.ui?.notify?.(
                                        `Config created at ${path.relative(cwd, file) || file}`,
                                        "info",
                                    );
                                else
                                    ctx.ui?.notify?.(
                                        `Project scope activated for ${path.basename(cwd)}`,
                                        "info",
                                    );
                            } else {
                                ...
                                ctx.ui.notify(
                                    changed
                                        ? "Project scope deactivated (flag set to false)"
                                        : `projectScope = ${value}`,
                                    "info",
                                );
                            }
                            ...
                            ctx.ui.notify(
                                `omp-zvec-grep: could not save ${id}: ${String((error as Error)?.message ?? error)}`,
                                "error",
                            );
                        }
                    } else {
                        ...
                        const targetScope: "user" | "project" = settings.projectScope
                            ? "project"
                            : "user";
                        try {
                            const { file: savedFile, created } = saveSettings(
                                settings,
                                targetScope,
                                cwd,
                            );
                            if (created)
                                ctx.ui.notify(
                                    `Config created at ${path.relative(cwd, savedFile) || savedFile}`,
                                    "info",
                                );
                            else ctx.ui.notify(`${id} = ${value}`, "info");
                        } catch (error) {
                            ctx.ui.notify(
                                `omp-zvec-grep: could not save ${id}: ${String((error as Error)?.message ?? error)}`,
                                "error",
                    ...
                    refreshScopeDisplay(items);
                    list.updateValue("defaultLimit", DEFAULT_LIMIT_DISPLAY(settings.defaultLimit));
                    list.updateValue("autoIndex", AUTO_INDEX_DISPLAY(settings.autoIndex));
                    list.invalidate();
                },
